...
export class ServiceProxy {
     ...
     * @return Success
     */
    updateBook(body: CreateBookDTO | undefined): Observable<CreateBookDTO> {
        let url_ = this.baseUrl + "/api/Catalog/UpdateBook";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processUpdateBook(response: HttpResponseBase): Observable<CreateBookDTO> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
